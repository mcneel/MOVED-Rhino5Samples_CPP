...
CRhinoCommand::result CCommandSampleAutomateGrasshopper::RunCommand( const CRhinoCommandContext& context )
{
  ...
  const CRhinoPlugInManager& plugin_manager = RhinoApp().PlugInManager();
  ON_ClassArray<CRhinoPlugInRecord> plugin_records( plugin_manager.PlugInCount() );
  plugin_manager.GetPlugInList( plugin_records );
...
#else
    CFileDialog dialog( TRUE, L"gh", 0, dwFlags, szFilter, pParentWnd );
#endif
  ...
  // This should probably go inCSampleCommandsPlugIn::OnLoadPlugIn...
